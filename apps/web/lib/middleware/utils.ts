...
import { SHORT_DOMAIN } from '@appoint/utils'
...
export const parse = (request: NextRequest) => {
	...
	// path is the path of the URL (e.g. appoint.co/dashboard/laudos -> /dashboard/laudos)
	...
	const key = decodeURIComponent(path.split('/')[1]) // key is the first path segment of the path (e.g. appoint.co/dashboard/laudos -> dashboard)
